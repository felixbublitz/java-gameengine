...
public class GameObject {
	...
	public Point getResultPosition(Point originalPosition){
		return new Point(Math.round(originalPosition.x *  this.game.getResFactor()),(Math.round(originalPosition.y *  this.game.getResFactor())));
	}

	public Dimension getResultSize(Dimension originalSize){
		return new Dimension(Math.round(originalSize.width *  this.game.getResFactor()),(Math.round(originalSize.height *  this.game.getResFactor())));
	}

	public Point getOriginalPosition(Point resultPosition){
		return new Point(Math.round(resultPosition.x /  this.game.getResFactor()),(Math.round(resultPosition.y /  this.game.getResFactor())));
	}

	public Dimension getOriginalSize(Dimension resultSize){
		return new Dimension(Math.round(resultSize.width /  this.game.getResFactor()),(Math.round(resultSize.height /  this.game.getResFactor())));
	}
	...
	protected void startDrawing(Graphics2D g){
		...
		if(this.interpolation != null && this.interpolation.getRotation() != 0){
			af.rotate(Math.toRadians(this.interpolation.getRotation()), this.postition.x+ this.size.width/2, this.postition.y+this.size.height/2);
		}else{
		...
		if(this.interpolation != null && this.interpolation.getAlpha() != 0){
			Color interPolColor = new Color(color.getRed(),color.getGreen(),color.getBlue(),this.interpolation.getAlpha());
